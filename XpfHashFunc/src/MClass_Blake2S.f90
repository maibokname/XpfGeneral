...
MODULE MClass_Blake2S

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *Blake2S* type and its related routines.
!   The *Blake2S* type is a *digest* type that directly extends from the
...
!   The *Blake2S* type implements an incremental cryptographic hash
...
!   *CreateHMAC* method instead of the *Create* method. <br>
!    <br>
!^ **REFERENCES**: <br>
...
!** MODULE PARAMETERS:
    ...
    tIndex,   PARAMETER :: DigestLen = 32_kIndex
    ...
    tIndex,   PARAMETER :: SIGMA(0:15,0:9) = RESHAPE([                      &
            ...
            10,  2,  8,  4,  7,  6,  1,  5, 15, 11,  9, 14,  3, 12, 13,  0], [16, 10])

!** DERIVED TYPE DEFINITIONS
    !> *Blake2S* is a concrete *digest* type that implements an incremental cryptographic
    ...
    CONTAINS
        ...
        PROCEDURE, PRIVATE  :: InitWOutLen  => Blake2S_InitWOutLen
        ...
        PROCEDURE       :: GetClone         => Blake2S_GetClone
        !> **Type-Bound Function**: GetName <br>
        !  **Purpose**:  To get the name of the *digest* object. <br>
        !  **Usage**: <br>
        !   --->    Name = MD%GetName()
        PROCEDURE       :: GetName          => Blake2S_GetName
...
SUBROUTINE Blake2S_InitDefault(MD)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To perform default initialization of the digest object.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(Blake2S), INTENT(INOUT)   :: MD    !! 'Blake2S' object
...
END SUBROUTINE Blake2S_InitDefault

!******************************************************************************

SUBROUTINE Blake2S_InitWOutLen(MD, OutputLen)
...
END SUBROUTINE Blake2S_InitWOutLen

!******************************************************************************

SUBROUTINE Blake2S_InitWKey(MD, Key, OutputLen)
...
END SUBROUTINE Blake2S_InitWKey

!******************************************************************************

SUBROUTINE Blake2S_Reset(MD)
...
! FLOW
    ...
    MD%T1 = 0
    MD%H(0:7) = IV(0:7)
    MD%H(0) = IEOR(MD%H(0), ToInt32(IOR(IOR(MD%GetDigestLen(), SHIFTL(MD%GetKeyLen(), 8)), ToIndex(Z'01010000'))))
    CALL MD%CoreReset()

    RETURN

END SUBROUTINE Blake2S_Reset

!******************************************************************************

SUBROUTINE Blake2S_GetClone(Src, Dst)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To clone the current state of the source object.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(Blake2S),                 INTENT(INOUT)   :: Src !! a source object
...
END SUBROUTINE Blake2S_GetClone

!******************************************************************************

FUNCTION Blake2S_GetName(MD) RESULT(Name)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To return the algorithm name of the hash function.
        
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(Blake2S), INTENT(IN)  :: MD       !! 'Blake2S' object
...
END FUNCTION Blake2S_GetName

!******************************************************************************

SUBROUTINE Blake2S_SetBufPtr(MD, BufPtr)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To set the pointer *BufPtr* to the actual buffer array with starting index of zero.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(Blake2S), TARGET, INTENT(INOUT)   :: MD           !! 'Blake2S' object
...
END SUBROUTINE Blake2S_SetBufPtr

!******************************************************************************

SUBROUTINE Blake2S_ProcessBlock(MD, BytesIn, LastBlock)
...
! FLOW
    ...
    IF ((MD%T0 == 0).AND.(MD%GetBufLen() > 0_kIndex)) MD%T1 = MD%T1 + 1
...
CONTAINS
    ...
    ! FLOW
        ...
        ! input block
        CALL BytePackLE(Input, 0_kIndex, M)
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        tInteger, INTENT(INOUT) :: V(0:15)
        tInteger, INTENT(IN)    :: A, B, C, D
        tInteger, INTENT(IN)    :: X, Y
    ...
    ! FLOW
        ...
        V(C) = V(C) + V(D)
        V(B) = RotateRight(IEOR(V(B), V(C)), 12)
        V(A) = V(A) + (V(B) + Y)
        V(D) = RotateRight(IEOR(V(D), V(A)), 8)
        V(C) = V(C) + V(D)
        V(B) = RotateRight(IEOR(V(B), V(C)), 7)
...
END SUBROUTINE Blake2S_ProcessBlock

!******************************************************************************

SUBROUTINE Blake2S_EncodeOutput(MD, BytesOut)
...
END SUBROUTINE Blake2S_EncodeOutput

!******************************************************************************

END MODULE MClass_Blake2S

...
MODULE MBase_SortDescend

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains routines that can be used to sort an array in a *descending*
...
!   See the <a href="../module/mbase_sortascend.html">MBase_SortAscend</a> module
!   for routines that perform sorting of an array in an *ascending* order.  Also, the
!   *MBase_SortAscend* module gives an overview of available sorting algorithms, which
!   are the same as the algorithms available in this module.  For ranking an array in
!   an *ascending* order, see the <a href="../module/mbase_ranking.html">MBase_Ranking</a>
!   module. <br>
...
!** MODULE PARAMETERS:
    tCharParam  :: ModName = 'MBase_SortDescend'
...
!** INTERFACE DEFINITIONS:
    ...
    END INTERFACE
    ! auxiliary
    INTERFACE IsSortedDescend
        !^ **Function Interface**: IsSortedDescend <br>
        !  **Purpose**:  To check whether the specified array is sorted in
        !       the desired order. <br>
        !  **Usage**: <br>
        !   --->    Flag = IsSortedDescend(A) <br>
        !   --->    IF (.NOT.IsSortedDescend(A)) DoSomething <br>
        MODULE FUNCTION IsSortedDescend_Char(A) RESULT(Flag)
            tCharStar, INTENT(IN)   :: A(:)
            tLogical                :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_I8(A) RESULT(Flag)
            tSInt8,  INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_I16(A) RESULT(Flag)
            tSInt16, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_I32(A) RESULT(Flag)
            tSInt32, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_I64(A) RESULT(Flag)
            tSInt64, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_R32(A) RESULT(Flag)
            tRealSP, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_R64(A) RESULT(Flag)
            tRealDP, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_R128(A) RESULT(Flag)
            tRealQP, INTENT(IN) :: A(:)
            tLogical            :: Flag
        END FUNCTION
        MODULE FUNCTION IsSortedDescend_Comp(A) RESULT(Flag)

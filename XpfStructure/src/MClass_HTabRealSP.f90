...
!   *REAL(KIND=kSingle)* type as the type of its stored keys and an unlimited polymorphic type
...
#define     KeyTypeA        REAL(KIND=kSingle)
#define     KeyTypeB        REAL(KIND=kSingle)
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'MClass_HTabRealSP'
    tSInt32,   PARAMETER    :: INFVAL  = ToInt32(Z'7F800000')  ! positive infinity
    tSInt32,   PARAMETER    :: NANVAL  = ToInt32(Z'7FC00000')  ! quiet NAN
    ! Special marker token used to indicate the deletion of a key-value pair
    tRealSP,   PARAMETER    :: DELKEY = TRANSFER(INFVAL, 1.0_kSingle)
    ! Special marker token used to indicate the empty of a key-value pair
    tRealSP,   PARAMETER    :: NULKEY = TRANSFER(NANVAL, 1.0_kSingle)

...
MODULE MClass_ListAnyType

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *ListAnyType* type and related routines. The
!   *ListAnyType* type is a container with an unlimited polymorphic type
...
!   Similar to other classes of the linked list container, the *ListAnyType*
...
!   that the size (in bytes) of the data to be stored is known at compile time. <br>
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'MClass_ListAnyType'
...
!** DERIVED TYPE DEFINITIONS
    ...
    !   The type of the item stored in this node can be any data type.  The *DLLNode* type
    !   is a private type.
    TYPE DLLNode
        PRIVATE
    ...
    END TYPE DLLNode
    !> *ListAnyType* is a container type that employs a doubly-linked list implementation to
    ...
    !   The type of items stored in this container can be any data type.
    TYPE ListAnyType
        PRIVATE
    ...
    CONTAINS
        ...
        !  **Usage**: <br>
        !   --->    Success = List%ToArray(Items, MakeCopy) <br>
        !   --->    IF (.NOT.List%ToArray(Items), MakeCopy) DoSomething <br>
        !  **Important Note**: This operation will not be successful if items stored in
        !   different nodes have different types.  Also, a user must supply a procedure
        !   to make a copy of an item for the specific type of stored items.
        ...
        !  **Usage**: <br>
        !   --->    CALL List%RemoveDuplicates(Compare) <br>
        !  **Important Note**: A user must supply a procedure to compare items stored
        !   in the list.
        ...
        !  **Usage**: <br>
        !   --->    Flag = List%GetAll(Items, MakeCopy) <br>
        !   --->    IF (.NOT.List%GetAll(Items), MakeCopy) DoSomething <br>
        !  **Important Note**: This operation will not be successful if items stored in
        !   different nodes have different types.  Also, a user must supply a procedure
        !   to make a copy of an item for the specific type of stored items.
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),     INTENT(INOUT)   :: List         !! ListAnyType object
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(*), OPTIONAL, ALLOCATABLE, INTENT(OUT)    :: Item     !! item of the removed node if requested
...
! FLOW
    ...
    ELSE
        ...
        IF (PRESENT(Item)) ALLOCATE(Item, SOURCE=CurrNode%Item)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(*),  INTENT(IN)      :: Items(N) !! an array of items
...
!** PURPOSE OF THIS SUBROUTINE:
    !^ To destruct ListAnyType object.
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType), INTENT(INOUT)   :: List     !! ListAnyType object
    CLASS(*),           INTENT(IN)      :: Item     !! item to be added to the list
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% ListAnyType object
    CLASS(ListAnyType), INTENT(INOUT)   :: List
...
FUNCTION LinkedList_ToArray(List, Items, MakeCopy) RESULT(Flag)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    CLASS(ListAnyType),    INTENT(INOUT)    :: List
    !% the item to be removed from the list
    CLASS(*), ALLOCATABLE, INTENT(INOUT)    :: Items(:)
    !% procedure to copy items
...
! FLOW
    ...
    ! get items
    Flag = List%GetAll(Items, MakeCopy)
...
SUBROUTINE LinkedList_RemoveDuplicates(List, Compare)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType), INTENT(INOUT)   :: List     !! ListAnyType object
    PROCEDURE(CompareData)              :: Compare  !! procedure to compare items

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    TYPE(DLLNode), POINTER  :: CurrNode
    TYPE(DLLNode), POINTER  :: NextNode
    CLASS(*),      POINTER  :: CurrItem
    CLASS(*),  ALLOCATABLE  :: IterItem
    tLogical                :: Repeat
    tLogical                :: IsTheEnd
    tLogical                :: Success
...
!** FLOW
    ...
    IF (ASSOCIATED(CurrNode)) ALLOCATE(IterItem, SOURCE=CurrNode%Item)
    ...
    DO WHILE (ASSOCIATED(NextNode))
        ...
        DO WHILE (.NOT.IsTheEnd)
            ...
            IF (Compare(CurrItem,IterItem) == 0) THEN
                Repeat = TrueVal
                EXIT
            END IF
    ...
    NULLIFY(NextNode)
    NULLIFY(CurrItem)
    IF (ALLOCATED(IterItem)) DEALLOCATE(IterItem)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),              INTENT(INOUT)  :: List     !! ListAnyType object
    CLASS(*), OPTIONAL, ALLOCATABLE, INTENT(OUT)    :: Item     !! first item
...
! FLOW
    ...
    IF (PRESENT(Item).AND.(.NOT.IsEmpty)) THEN
       ALLOCATE(Item, SOURCE=List%Cursor%Item)
    END IF
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),              INTENT(INOUT)  :: List     !! ListAnyType object
    CLASS(*), OPTIONAL, ALLOCATABLE, INTENT(OUT)    :: Item     !! last item
    tLogical                                        :: IsEmpty  !! true if the list is empty
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),              INTENT(INOUT)  :: List     !! ListAnyType object
    CLASS(*), OPTIONAL, ALLOCATABLE, INTENT(OUT)    :: Item     !! item of the next node
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),              INTENT(INOUT)  :: List     !! ListAnyType object
    CLASS(*), OPTIONAL, ALLOCATABLE, INTENT(OUT)    :: Item     !! item of the previous node
    tLogical                                        :: IsTheEnd !! true if the cursor has reached the end of the list
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType), INTENT(IN)  :: List !! ListAnyType object
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% ListAnyType object
    CLASS(ListAnyType),     INTENT(IN)  :: List
...
! FLOW
    ...
    Flag = TrueVal
    ALLOCATE(Item, SOURCE=List%Head%Item)
...
! FLOW
    ...
    Flag = TrueVal
    ALLOCATE(Item, SOURCE=List%Tail%Item)
...
! FLOW
    ...
    IF (ASSOCIATED(CurrNode)) THEN
        Flag = TrueVal
        ALLOCATE(Item, SOURCE=CurrNode%Item)
    ELSE
        ! set output flag
        Flag = FalseVal
    END IF
...
FUNCTION LinkedList_GetAllItems(List, Items, MakeCopy) RESULT(Flag)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(ListAnyType),    INTENT(INOUT)    :: List     !! ListAnyType object
    CLASS(*), ALLOCATABLE, INTENT(OUT)      :: Items(:) !! an allocatable array of items
    PROCEDURE(CopyData)                     :: MakeCopy !! procedure to copy items
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(ListAnyType), INTENT(INOUT)    :: List  !! ListAnyType object

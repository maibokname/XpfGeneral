...
MODULE MClass_TreeInteger1B

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *TreeInteger1B* type and its related helper type and routines.
...
!   operations for an ordered symbol table.  As an ordered symbol table, the *TreeInteger1B*
!   type uses the Fortran intrinsic *INTEGER(KIND=kInt8)* type as the type of its stored keys
!   and an unlimited polymorphic type as the type of its stored values. <br>
!   As a symbol table, the *TreeInteger1B* type does not allow duplicated keys.  Therefore,
...
!   *TreeInteger1B* type employs a left-leaning red-black (RB) tree as the balanced BST. <br>
...
#define KeyTypeA        INTEGER(KIND=kInt8)
#define KeyTypeB        INTEGER(KIND=kInt8)
#define KeyTypeC        INTEGER(KIND=kInt8)
#define QueueKey        ListInteger1B
#define QueueVal        ListAnyType
#define RedBlackTree    TreeInteger1B

!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'MClass_TreeInteger1B'
...
END MODULE MClass_TreeInteger1B

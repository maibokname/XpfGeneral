...
!   *REAL(KIND=kQuad)* type as the type of its stored keys and an unlimited polymorphic type
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'MClass_HTabRealQP'
    ! positive infinity (assuming little-endian order)
    tSInt64,   PARAMETER    :: INFVAL(2) = [ToInt64(Z'0000000000000000'), ToInt64(Z'7FFF000000000000')]
    ! quiet NAN (assuming little-endian order)
    tSInt64,   PARAMETER    :: NANVAL(2) = [ToInt64(Z'0000000000000000'), ToInt64(Z'7FFF800000000000')]
    ! Special marker token used to indicate the deletion of a key-value pair
    tRealQP,   PARAMETER    :: DELKEY = TRANSFER(INFVAL, 1.0_kQuad)
    ! Special marker token used to indicate the empty of a key-value pair
    tRealQP,   PARAMETER    :: NULKEY = TRANSFER(NANVAL, 1.0_kQuad)

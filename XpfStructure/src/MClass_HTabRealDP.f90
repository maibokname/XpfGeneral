...
MODULE MClass_HTabRealDP

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *HTabRealDP* type, the *TabItem* type and their
...
!   As an unordered symbol table, the *HTabRealDP* type uses the Fortran intrinsic
!   *REAL(KIND=kDouble)* type as the type of its stored keys and an unlimited polymorphic type
...
!   Technically, the *HTabRealDP* type employs the open-addressing as a collision
...
!** MODULE PARAMETERS:
    ! name of module
    tCharStar, PARAMETER    :: ModName = 'MClass_HTabRealDP'
    tSInt64,   PARAMETER    :: INFVAL  = ToInt64(Z'7FF0000000000000')  ! positive infinity
    tSInt64,   PARAMETER    :: NANVAL  = ToInt64(Z'7FF8000000000000')  ! quiet NAN
    ! Special marker token used to indicate the deletion of a key-value pair
    tRealDP,   PARAMETER    :: DELKEY = TRANSFER(INFVAL, 1.0_kDouble)
    ! Special marker token used to indicate the empty of a key-value pair
    tRealDP,   PARAMETER    :: NULKEY = TRANSFER(NANVAL, 1.0_kDouble)

...
!** MODULE PARAMETERS:
    tCharStar, PARAMETER    :: ModName = 'MClass_PQCharacter'
...
! FLOW
    ...
    CALL MemAlloc(PQ%Keys, PQ%CharLen, [Capacity])
...
! FLOW
    ...
    IF (.NOT.PQ%IsEmpty()) THEN
        IF (PRESENT(Keys)) THEN
            ...
            N = PQ%Last
            CALL MemAlloc(Keys, PQ%CharLen, [N])
            DO ID = 1_kIndex, N

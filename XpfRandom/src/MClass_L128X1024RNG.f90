...
MODULE MClass_L128X1024RNG

!^ **PURPOSE OF THIS MODULE**: <br>
!   This module contains the *L128X1024RNG* type and its related routines.
...
!   In particular, the *L128X1024RNG* type provides an implementation of the
!   *NextLongImpl* deferred procedure based on the *L128X1024* algorithm.  The
!   *L128X1024* algorithm is a specific member of the LXM family of algorithms
...
!   -  M stands for Mix. <br>
!   The *L128X1024RNG* type employs a 128-bit LCG, a 1024-bit XBG and a mixing function
!   where its memory footprint is 1312 bits and its period is roughly 2<sup>1152</sup>. <br>
!   In addition to common operations of a PRNG, the *L128X1024RNG* type provides
...
!   It is important to note that the *L128X1024* PRNG requires an explicit
...
!** DERIVED TYPE DEFINITIONS
    ...
    CONTAINS
        ...
        PROCEDURE       :: BaseInit         => L128X1024RNG_BaseInit
        ...
        PROCEDURE       :: NextLongImpl     => L128X1024RNG_NextLong
        !> *GetName* is a deferred procedure. <br>
        !  **Type-Bound Function**: GetName <br>
        !  **Purpose**:  To get the name of the PRNG. <br>
        !  **Usage**: <br>
        !   --->    Name = PRNG%GetName()
        PROCEDURE       :: GetName          => L128X1024RNG_GetName
        ...
        PROCEDURE       :: GetSeedSize      => L128X1024RNG_GetSeedSize
        ! ---------------------------------------------------------------------
        ! -----                 Public Procedures                         -----
        ! ---------------------------------------------------------------------
        ...
        PROCEDURE       :: Split            => L128X1024RNG_Split
        ! ---------------------------------------------------------------------
    END TYPE L128X1024RNG
...
SUBROUTINE L128X1024RNG_BaseInit(RNG, Seed)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To initialize the 'L128X1024RNG' object with specified seed(s).

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(L128X1024RNG), INTENT(INOUT)  :: RNG      !! 'L128X1024RNG' object
    tSInt64,             INTENT(IN)     :: Seed(:)  !! seed(s)

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tSInt64     :: InitSeed1, InitSeed2, X(0:15), XSeed
    tIndex      :: I
...
! FLOW
    ...
    END DO
    CALL L128X1024RNG_SetParameters(RNG, Mix_Murmur(InitSeed1), Mix_Murmur(InitSeed2), &
                                    0_kInt64, 1_kInt64, X)

    RETURN

END SUBROUTINE L128X1024RNG_BaseInit

!******************************************************************************

FUNCTION L128X1024RNG_NextLong(RNG) RESULT(RandNum)
...
END FUNCTION L128X1024RNG_NextLong

!******************************************************************************

FUNCTION L128X1024RNG_GetName(RNG) RESULT(Name)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To return the name of the generator.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(L128X1024RNG), INTENT(IN) :: RNG      !! 'L128X1024RNG' object
...
END FUNCTION L128X1024RNG_GetName

!******************************************************************************

FUNCTION L128X1024RNG_GetSeedSize(RNG) RESULT(Size)
...
END FUNCTION L128X1024RNG_GetSeedSize

!******************************************************************************

SUBROUTINE L128X1024RNG_SetParameters(RNG, AH, AL, SH, SL, X)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To set parameters of the object.

!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(L128X1024RNG), INTENT(INOUT)   :: RNG
...
END SUBROUTINE L128X1024RNG_SetParameters

!******************************************************************************

FUNCTION L128X1024RNG_Split(RNG, Brine) RESULT(NewRNG)

!** PURPOSE OF THIS SUBROUTINE:
    !^ To initialize and return a new 'L128X1024RNG' object that shares no mutable
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    !% 'L128X1024RNG' object
    CLASS(L128X1024RNG), INTENT(INOUT)  :: RNG
    ...
    !% new 'L128X1024RNG' object
    TYPE(L128X1024RNG)                  :: NewRNG

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    tSInt64     :: Salt, AH, SH, SL, X(0:15)
    tIndex      :: I
...
! FLOW
    ...
    DO I = 0, 15
        X(I) = RNG%NextLong()
    END DO
    CALL L128X1024RNG_SetParameters(NewRNG, AH, SHIFTL(Salt, 1), SH, SL, X)

    RETURN

END FUNCTION L128X1024RNG_Split

!******************************************************************************

END MODULE MClass_L128X1024RNG
